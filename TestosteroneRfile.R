...
#logistic regression analysis with Testosterone as IV
...
logitmodel <- glm(AcnePresent ~ Testosterone, data=DATA, family="binomial")
#create predictions
...
#accuracy rate is 83% which is good
...
#testosterone graph shows line with slope so assumption is met
...
#graph doesn't entirely have even distribution of points across the x axis so assumption not met
...
#run regression
summary(logitmodel)
#p value is < .05 so amount of testosterone is a significant predictor of women having acne
#in layman's terms, for every one unit increase in testosterone, the log odds of having acne increases by 0.06016
